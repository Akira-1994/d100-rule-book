...
- [x] `src-tauri/src/rules.rs`：公式與 `tools/rules.py` 對齊，回歸測試的基準
      值與 `tools/validate.py` 相同（創角色須知原文的難度 1 各級列舉）
- [x] 附錄章節的試算頁：可直接輸入難度或從 612 條專長挑一條；
      傳奇計價同時顯示傳奇技能點與等值 CP；`1or2` 提示需 DM 裁定
